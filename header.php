    ...
    <header id="masthead" class="site-header" role="banner">
        ...
        <div class="site-logo">
          <a href="<?php echo esc_url( home_url( '/' ) ); ?>"
            title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><img
              src="<?php echo get_theme_mod( 'themeslug_logo' ); ?>"
              alt="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>"></a>
        </div>
        <?php else : ?>
        <?php $heading_tag = ( is_home() || is_front_page() ) ? 'h1' : 'div'; ?>
        <<?php echo $heading_tag; ?> class="site-title"><a href="<?php echo home_url( '/' ); ?>"
            title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>"
            rel="home"><?php bloginfo( 'name' ); ?></a></<?php echo $heading_tag; ?>>
      ...
      <div id="cart-navigation">
        ...
        <ul>
          ...
          <li class="myaccount"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>"
              title="<?php _e('My Account','woothemes'); ?>"><i class="ion-person"></i></a></li>
          <?php }
		else { ?>
          <li class="login"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>"
              title="<?php _e('Login / Register','woothemes'); ?>"><?php _e('Login','woothemes'); ?></a></li>
          <?php } ?>

          <li id="cd-cart-trigger"
            class="view-cart cartis<?php echo sprintf( _n( '%d', '%d', WC()->cart->get_cart_contents_count(), '' ), WC()->cart->get_cart_contents_count() );?>">
            <a class="cd-img-replace" href="#0">
              <i class="ion-bag"><span
                  class="cart-count"><?php if ( WC()->cart->get_cart_contents_count() !== 0 ) {?><span
                    class="count"><?php echo sprintf( _n( '%d', '%d', WC()->cart->get_cart_contents_count(), '' ), WC()->cart->get_cart_contents_count() );?></span><?php } ?></span></i>
            </a>
          </li>
    ...
    <div id="main">
      ...
      <div class="topbanner">
        ...
        <div class="featured-container">
          <div class="featured01"><a
              href="<?php echo get_option( 'haku_feature_banner01_link' ); ?>"><?php echo_feature_banner01(); ?></a>
          </div>
          <div class="featured02"><a
              href="<?php echo get_option( 'haku_feature_banner02_link' ); ?>"><?php echo_feature_banner02(); ?></a>
          </div>
        </div>
        <?php endif; ?>
